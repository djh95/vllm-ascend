...
`--async-scheduling`：token/logprob 在 `_bookkeeping_sync` 时尚在 device 上，检测改在 `AscendAsyncGPUModelRunnerOutput.get_output()`（D2H 完成并 parse 之后）执行；仍仅 last PP 处理，`detect`/`hit` 只在该 PP 的 TP0 打印。
...
| `vllm_ascend/worker/model_runner_v1.py` | sync：采样后直接 `check_all_token_logprobs`；async：`AscendAsyncGPUModelRunnerOutput.get_output()` 在 D2H 后检测 |
