...
| 3. Dump / 观测开关 | `dumper/`（`Dumper` + mixins） | msprobe dump 生命周期（**可选依赖**：缺包则软失败并强制 dump 关闭）；`ascend_log` 开关 |
...
| 6. Input filter | `input_filters.py`（`InputFilterManager`） | detect 前输入过滤（单例；`manual_dump` / `ManualTriggerManager` 不走） |
...
- `DfxProcessor` 在异常检测命中或手动 dump（`manual_dump`）命中后，都会尝试调用 dumper；**无论 dump 是否成功 arm，都会写 report**，避免 dump 配额耗尽、冷却中或相关性校验失败时丢失异常证据。
...
- `save()` / `manual_dump` 递减清盘：仅 JSON writer；非 writer 忽略写盘。
...
| 段 | 含义 |
|----|------|
| `dump` | `auto_max_times`（auto-arm 配额，`>0` 开）/ `auto_cooldown_seconds` / `manual_dump`（手动 arm，见运维页）/ `msprobe_config_path` / `reload_msprobe`。runtime **dump 激活** = `auto_max_times>0` 或 `manual_dump` 有效；`auto` 与 `manual` **互斥**。与 detector 正交。旧键 `enabled`/`max_times`/`manual_trigger`/`cooldown_seconds` 已移除 |
...
- **`manual_dump`**：依赖热更；`dfx_config_reload_interval` 必须 `> 0`。操作与排障见 [dfx_ops.md](./dfx_ops.md) §1.1 / §2.2。
- **msprobe bootstrap**：无 `dump` 段 + msprobe 默认 on → seed `manual_dump=true`；显式 `dump` 段 off 与 msprobe 冲突时报错或写回 msprobe。详见 [dfx_ops.md](./dfx_ops.md) §1.1。
...
配置在顶层 `input_filter.filters`（**检测阶段**；不挡 dump arm / `manual_dump`）。
...
| `manual_dump` / `ManualTriggerManager` | **不**调用 `InputFilterManager` |
...
- 运维 / `manual_dump` / 过滤：[dfx_ops.md](./dfx_ops.md)
...
3. `sync_dump_pending_or()`（仅 last-PP TP；**不含** config / report）。热更关且 dump 未激活时走 fast-path，跳过 TP OR（见 [dumper_design.md](./dumper_design.md) §5）；**同一 EngineCore 内各 TP 的 dump 激活态必须一致**，否则可能一侧 skip、一侧 `all_reduce` 挂死。

Dumper **不**调用 config reload，也 **不**写 report，也 **不**刷新 `InputFilterManager`（均在 processor）。

门控（异常检测）：至少一路 `detector.<name>.enabled` 开，且 rank 合法；**不依赖** dump 激活 / `auto_max_times`。另受 §2.6 过滤。dump 激活且 dump pending/active 时跳过再检，避免重叠 arm。  
门控（自动 dump）：dump 激活 + `auto_max_times>0` + 配额 / 冷却；`auto_max_times == 0` 时不 auto-arm（detect / `manual_dump` 仍可）。  
约束：detect 与 dump **正交** — 可 detect-only、detect+dump、或 **manual-only**（`manual_dump` 开、无 detector）。无 detector 时 dump 开只会 warn，不强制改配置。  
`manual_dump` 由 `ManualTriggerManager` 消费；要求 dump 激活 + debugger，**不要求** detector；不受 `auto_max_times` / cooldown / InputFilterManager 限制。dump 未激活时不消费。`true` = 持续 arm（改回 `false` 才停）；正整数 = 剩余拍数。  
手动 dump arm 成功后写 **一份** `manual_trigger` 类型 report：`detail.requests[]` 快照当前 batch **全部**请求的 I/O（与单请求 anomaly report 不同；普通异常触发 dump 仍只写单请求 report）。  
**前提**：`additional_config.dfx_config_reload_interval > 0`（热更为关时改 JSON 的 `manual_dump` 不会生效）。
...
- 何时写 anomaly：
  - dump 未激活（detect-only）：检测到异常即写 report（无 `_dump`）
  - dump 激活：尝试 arm dump；**无论 arm 成功与否都写 report**（arm 失败——如配额耗尽——不吞掉检测证据；仅成功时文件名带 `_dump`）
