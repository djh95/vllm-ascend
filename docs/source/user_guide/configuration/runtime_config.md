...
> Online KV meta detectors (`block_kv` / `slot_consistency` / `position_alignment`)
> and offline compare scripts are deferred to a follow-up PR. This release still
> supports native `dump_kv` capture for later offline use.
