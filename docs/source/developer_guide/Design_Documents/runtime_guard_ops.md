...
- 前置检查：`type(logging.getLogger("vllm_ascend.runtime_guard..."))` 应为 `logging.Logger`，且在 `Logger.manager.loggerDict` 中（详见 `SYSTEM_OBSERVABILITY_TEST.md` / TEST_MATRIX L1–L5）。
