        ...
        *,
        ...
        try:
            ...
            record = {
                ...
                # Deterministic dump landing dir for this incident; files appear
                # under wave_* on a later sync after the dump job runs.
                "dump_dir": str(
                    dump_root
                    / str(incident_type or "unknown")
                    / str(req_id or "unknown")
                    / (
                        f"wave_{int(dump_arm_wave)}"
                        if dump_arm_wave is not None
                        else "wave_unknown"
                    )
                ),
