...
def need_pre_sample_hook(guard: Any) -> bool:
    """True when wrapping ``compute_logits`` is useful (invariant on + gate open)."""
...
def wrap_compute_logits_for_pre_sample(runner: Any, input_batch: Any):
    ...
    The check fires once per context: v2 ``sample_tokens`` calls
    ``compute_logits`` twice per step (sampling + prompt logprobs), and
    duplicate checks would double detection work per step.
    """
    model = runner.model
