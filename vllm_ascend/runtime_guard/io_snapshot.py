...
"""Report-time I/O views over :class:`RequestGuardStore` + runner prompts.

Ownership split:

- **Data** (cumulative ``output_token_ids``, append dedupe): ``RequestGuardStore``
...
from vllm_ascend.runtime_guard.input_filters import prompt_token_ids_for_request
...
class RequestIoSnapshotManager:
    ...
    def clear_req(self, req_id: str) -> None:
        """Compatibility: finish cleanup via :meth:`RequestGuardStore.clear`."""
        if not req_id:
            return
        RequestGuardStore.get().clear(str(req_id))

    def cumulative_output_ids(self, req_id: str) -> list[int]:
        st = RequestGuardStore.get().get_state(req_id)
        return list(st.output_token_ids) if st is not None else []

    def cumulative_output_count(self, req_id: str) -> int:
        st = RequestGuardStore.get().get_state(req_id)
        return len(st.output_token_ids) if st is not None else 0
