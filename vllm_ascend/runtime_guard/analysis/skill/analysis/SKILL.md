...
本 skill / 脚本挂在 **`feat/runtime-guard-analysis`**：只做离线后处理与测试附属物，
不承载 `feat/runtime-guard-config` 上的产品改动。落盘布局以 config 运行时为准，见
`vllm_ascend/runtime_guard/analysis/README.md`。
...
| **输出** | 匹配的 report 路径、`dump_attempted` / `block_ids`、`kv_cache/.../<req_id>/wave_*/<rank_tag>/`；并打印下一步 `verify_request_kv` 命令 |
| **判据** | `reports>=1`；若 `dump_attempted` 但 kv 目录缺失 → 记为配额/路径问题 |
...
| **输入** | `--report <report_*.json>`；可选 `--report-dir`、`--kv-dir`、`--block-size`、`--min-files`。`--kv-dir` 语义是 **rank 目录**（`…/wave_N/<rank_tag>/`，直接含 `.pt`）；传更高层目录会自动下钻到各 rank dir 逐个校验 |
| **输出** | 分段检查日志；结论 `PASS` 或 `FAIL`（exit 0/1）；层数按 K/V 去重（`layers=N kv_files=M`） |
...
1. `incident_type`、`req_id`、`dump_attempted` / `dump_arm_wave`（若有）
...
以 config 产品写盘为准（详见 `analysis/README.md`）：

```
runtime/report/
  <incident_type>/report_*.json
  kv_cache/<incident_type>/<req_id>/wave_<N>/<rank_tag>/*.pt
  kv_cache/<incident_type>/<req_id>/wave_<N>/request_info.json
```

默认是 **本请求 `block_ids` 快照**（`[n_blocks, block_size, H, D]`），不是全 cache、不是 op 级 dump。
旧文档里的扁平 `.../<req_id>/*.pt`、`shared/`、`dump_attempted`、 已废弃。
