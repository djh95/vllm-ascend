...
def resolve_kv_dump_dir(
    ...
    *,
    ...
    Prefer ``report["dump_dir"]`` + ``report["rank"]``; then scan
    ``kv_cache/<type>/<req_id>/wave_*``; finally accept legacy flat
    ``kv_cache/<type>/<req_id>/`` if it already contains ``*.pt``.
    """
    rank_tag = str(rank if rank is not None else (report.get("rank") or "")).strip()
    dump_dir_raw = report.get("dump_dir")
    if dump_dir_raw:
        base = Path(str(dump_dir_raw))
        if rank_tag:
    ...
    req_id = str(report.get("req_id") or "unknown")
    if report_dir is None:
        raise ValueError("resolve_kv_dump_dir needs report['dump_dir'] or report_dir=")
    req_root = Path(report_dir) / "kv_cache" / itype / req_id
    wave = report.get("dump_arm_wave")
    wave_dirs: list[Path] = []
    if wave is not None:
        wave_dirs.append(req_root / f"wave_{int(wave)}")
    if req_root.is_dir():
        wave_dirs.extend(sorted((p for p in req_root.glob("wave_*") if p.is_dir()), reverse=True))
    seen: set[Path] = set()
    for wave_dir in wave_dirs:
        resolved = wave_dir.resolve()
        if resolved in seen:
            continue
        seen.add(resolved)
        if rank_tag:
            ranked = wave_dir / rank_tag
            hit = _first_pt_subdir(ranked)
            if hit is not None:
                return hit
        hit = _first_pt_subdir(wave_dir)
        if hit is not None:
            return hit
    flat = _first_pt_subdir(req_root)
    if flat is not None:
        return flat
    if wave is not None:
        return req_root / f"wave_{int(wave)}" / (rank_tag or "unknown_rank")
    return req_root / (rank_tag or "")
