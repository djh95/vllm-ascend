...
- 层文件名带 `[0]`/`[1]`（K/V）后缀，shell/`glob` 里方括号是**字符类**——自定义分析按**子串**匹配文件名，别用 glob。
- 跨 TP/PP 数值对比阈值默认 **0.999**（TP 切分 bf16 all-reduce 顺序噪声 ~0.999–0.9999，0.9999 会误报）；少 token wave 用 `--written-only`/`--max-tokens` 只比已写 slot 并看 `slots` 列（见 RUN_NOTES 2026-09-16）。
