...
class ManualTriggerManager:
    ...
    def consume_once(
        ...
        *,
        ...
        if not allow_arm:
            logger.debug(
                "[runtime_guard manual_trigger] dump.manual_trigger deferred (allow_arm=False); "
        ...
        if not self._wave_has_scheduled_tokens(scheduler_output):
            logger.debug(
                "[runtime_guard manual_trigger] dump.manual_trigger deferred (no scheduled tokens); remaining=%d",
                remaining,
            )
            return None
