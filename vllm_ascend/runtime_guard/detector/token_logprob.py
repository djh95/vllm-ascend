...
from vllm_ascend.runtime_guard.io_snapshot import output_token_count_for_request, prompt_token_count_for_request
...
class TokenLogprobDetector(ConfigBackedDetector):
    ...
    incident_type = "token_logprob"
    EST_COST_PER_STEP = 1.5  # window + ILL processing (top-k probe)
    ...
    def _apply_detector_values(self, getter: Callable[[str, Any], Any]) -> None:
        ...
        # crash on every apply_runtime_config. Wrap each cast; keep prior on failure.
        ...
        if self._window != old_window:
            self._rebuild_window_buffers()

    def _rebuild_window_buffers(self) -> None:
        # A3: deque(maxlen) is fixed at construction — a hot window resize must
        # rebuild live buffers (keeping the newest tokens) and reset
        # window-scoped hit counters, or old widths silently stick around.
        window = self._window
        for rid in list(self._buf):
            self._buf[rid] = deque(self._buf[rid], maxlen=window)
        self._ill_window_hits.clear()
    ...
    def check_one(
        ...
        alert = Incident.from_ill_result(
            req_id=req_id,
            result=result,
            req_idx=req_idx,
        ...
        try:
            ...
            if cu is not None:
                ...
                if num_tokens == 1:
                    ...
                    # A2: without cu_num_generated_tokens the per-request row
                    # counts are unknown under MTP (variable accept lengths).
