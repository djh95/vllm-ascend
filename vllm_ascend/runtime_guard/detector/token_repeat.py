...
from vllm_ascend.runtime_guard.util import normalize_token_ids
...
class TokenRepeatDetector(ConfigBackedDetector):
    ...
    def check_all(
        ...
        new_ids_by_req: dict[str, list[int]] | None = None,
    ) -> list[Incident]:
        ...
        for batch_idx, req_id in enumerate(req_ids):
            if not req_id:
                continue
            if new_ids_by_req is not None:
                new_ids = list(new_ids_by_req.get(req_id) or [])
                self._consumed_len[req_id] = self._consumed_len.get(req_id, 0) + len(new_ids)
            else:
