...
from vllm_ascend.runtime_guard.detector.base import AnomalyDetector, DetectorRegistry
...
from vllm_ascend.runtime_guard.util import freeze_sampled_rows
...
class DetectorManager:
    ...
    def check_after_spec(
        ...
        return self._spec_det.check_all(sampled_tokens, accepted_token_nums, skip_req_ids=skip)
    ...
    def run_after_sample_cpu(self, snap: AfterSampleCpuSnapshot) -> list[Incident]:
        """output_substring → token_repeat on a frozen snapshot."""
        ...
        new_ids_by_req = {
            rid: snap.sampled_token_ids[i]
            for i, rid in enumerate(resolved_ids)
            if rid and i < len(snap.sampled_token_ids)
        }
        alerts.extend(
