...
from vllm_ascend.runtime_guard.detector.position_alignment import query_start_loc_np
...
class LogitsFiniteDetector(ConfigBackedDetector):
    ...
    EST_COST_PER_STEP = 2.0  # full [n_reqs, vocab] device scan
        ...
        *,
        ...
        for row_t in bad_rows.tolist():
            ...
            if num_reqs > 0 and row < num_reqs and not idx_list:
                ...
                req_id = req_ids[row]
            if not req_id:
                # A1: row→request mapping unavailable (e.g. chunked prefill with
                # logits_indices but no qsl). Never guess and never drop: the
                # NaN must alert even unattributed.
