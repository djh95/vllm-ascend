...
def classify_violation(slice_pos: "np.ndarray", expected_start: int) -> str:
    """Label a misaligned position span: ``""`` / ``wrong_start`` /
    ``non_consecutive`` / ``misaligned``.

    ``wrong_start``: contiguous run but offset from ``expected_start``.
    ``non_consecutive``: ascending with gaps (dominates a wrong start).
    ``misaligned``: duplicates / regression / descending, or empty span.
    """
    if slice_pos.size == 0:
        return "misaligned"
    start_ok = int(slice_pos[0]) == int(expected_start)
    if slice_pos.size == 1:
        return "" if start_ok else "wrong_start"
    diffs = np.diff(slice_pos.astype(np.int64))
    if bool(np.all(diffs == 1)):
        return "" if start_ok else "wrong_start"
    if bool(np.all(diffs >= 1)):
...
class PositionAlignmentDetector(ConfigBackedDetector):
    ...
    incident_type = "position_alignment"
    EST_COST_PER_STEP = 1.0
        ...
        *,
        ...
        for req_id, req_idx, start, end, expected_start, scheduled in spans:
            ...
            if np.array_equal(slice_pos, expected):
                continue
            violation = classify_violation(slice_pos, expected_start)
            alerts.append(
                Incident(
