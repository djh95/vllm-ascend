...
class DumpKvAction(Action):
    ...
    def prepare(self, ctx: ActionContext) -> None:
        ...
        if not targets:
            ...
            _record_dump_skipped(ctx, reason="no_dump_targets", detail={"scope": scope})
            return None
        ...
        if not ctx.quota.try_consume(consume_quota=ctx.incident.consume_quota):
            ...
            _record_dump_skipped(ctx, reason="quota_or_cooldown_blocked")
            return None
...
def _record_dump_skipped(
    ...
    *,
    ...
    """Persist dump-skip reason under dump_root (last-PP TP0 / action leader)."""
    write_kv_dump_skipped(
        ctx.runtime_config.dump_root(),
