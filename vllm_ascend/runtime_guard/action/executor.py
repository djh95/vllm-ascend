...
class ActionExecutor:
    ...
    def handle(
        ...
        *,
        ...
        # Manual dump always arms KV for the live batch (scope forced in DumpKvAction).
        if incident.incident_type == MANUAL_TRIGGER_TYPE and "dump_kv" not in names:
            names.append("dump_kv")
        overrides = dict(det_cfg)
        if incident.incident_type == MANUAL_TRIGGER_TYPE:
