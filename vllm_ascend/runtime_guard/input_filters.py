...
from vllm_ascend.runtime_config._filters import MODE_EXCLUDE, MODE_INCLUDE
...
def _prompt_ids_from_req_states(
    ...
    if row is None:
        ...
        try:
            ...
            return None
    # S13 fix: Bug #9 fallback path. If host mirror is all-zero (n>0 but
    # every entry 0), the StagedWriteTensor hasn't been committed by
    # apply_staged_writes yet. Returning [0,0,...,0] silently poisons
    # downstream filters/snapshots. Return None so the caller falls back
    # to scheduler_output or marks the prompt unavailable.
    # R3: a tokenizer-valid prompt of all token id 0 would also return None;
    # we accept that rare edge in favour of detecting uncommitted mirrors.
    try:
        row_list = row.tolist() if hasattr(row, "tolist") else list(row)
    except Exception:
        return None
    if n > 0 and all(int(x) == 0 for x in row_list):
        return None
...
def prompt_token_ids_for_request(
    ...
    # None. The cache survives both windows.
...
class InputFilterManager:
    ...
    def apply_configs(self, configs: Sequence[dict[str, Any]] | None) -> bool:
        ...
        RequestGuardStore.get().clear_all_filter_allowed()
        return True

    def apply_from_config(self, runtime_config: Any | None) -> bool:
        """Pull ``input_filter.filters`` from live config; skip if unchanged."""
        configs: list[dict[str, Any]] = []
        if runtime_config is not None:
            getter = getattr(runtime_config, "input_filter_configs", None)
            if callable(getter):
                raw = getter()
    ...
    def clear_req(self, req_id: str) -> None:
        ...
        Prefer :meth:`RequestGuardStore.clear` from ``RuntimeGuardProcessor._reap_finished_requests`` / Store.clear.
        """
        RequestGuardStore.get().set_filter_allowed(req_id, None)
    ...
    def allow(
        ...
        *,
        ...
        cached = None if state is None else state.filter_allowed
        ...
        if ids is None:
            if log:
                logger.debug(
                    "[runtime_guard filter] skip detect req_id=%s: filters set but prompt_token_ids unavailable",
                    req_id,
                )
            return False
        ...
        if not ok and log:
            logger.debug(
                "[runtime_guard filter] skip detect req_id=%s: input filter reject prompt_len=%d filters=%d",
                req_id,
                len(ids),
