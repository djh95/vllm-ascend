# runtime_guard / runtime_config 功能测试清单（细粒度）

> 本文档是 **功能测试（e2e）** 的权威清单，与 `TEST_MATRIX.md`（UT/冒烟矩阵）、
> `perf/README.md`（性能 T0–T5 / C1–C5）并列。
> 三者分工：
> - **TEST_MATRIX.md** — 单元/白盒回归 + 日志绕过（L1–L5），CPU 侧可跑；
> - **perf README.md** — 吞吐/开销对比（需要 live NPU，交叉轮换）；
> - **本文档** — 功能行为是否符合预期（需要 live NPU + 模型权重 + 多卡/多节点拓扑）。

功能测试回答的是「功能对不对」，不是「跑得快不快」——两者正交，但共享同一套
拓扑/模型/长度覆盖要求（v2 优先再 v1、M1/M2/T4/T5/T6）。
