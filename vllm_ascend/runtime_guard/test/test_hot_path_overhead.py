...
def test_broadcast_all_reduce_every_call_even_when_not_due(tmp_path: Path):
    ...
    with patch("torch.distributed.all_reduce") as ar:
        assert cfg._maybe_reload_broadcast(group) == (False, [])
        ar.assert_called_once()
        group.broadcast_object.assert_not_called()


def test_dump_extra_due_broadcasts_without_advancing_config_interval(tmp_path: Path):
    """Dump hitchhike: extra_due triggers object broadcast; config clock stays."""
    cfg = _cfg(tmp_path, reload=5.0)
    cfg._initial_broadcast_done = True
    ts = time.time()
    cfg._last_reload_ts = ts
    ...
    group.broadcast_object.side_effect = _bcast
    jobs = [{"req_id": "r1", "incident_type": "token_repeat"}]
    with (
        patch("torch.distributed.all_reduce"),
        patch.object(cfg, "reload") as reload_fn,
    ):
        changed, extra = cfg._maybe_reload_broadcast(
            group, extra_due=True, extra=jobs
        )
    assert changed is False
    assert extra == jobs
    reload_fn.assert_not_called()
    assert captured["obj"]["skip_config"] is True
    assert captured["obj"]["dump_jobs"] == jobs
    assert cfg._last_reload_ts == ts
