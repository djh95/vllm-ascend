...
ROOT=/data0/test-mrv2-cann91/rg_tip12_matrix
LOG=$ROOT/master.log
PORT=8145
mkdir -p "$ROOT"
...
residual_check(){
  # Evidence for orphan processes left after a state stops (zombie-reap attribution).
...
for entry in "${MATRIX[@]}"; do
  ...
  for st in T1 T2 T3; do
    ...
    if [ "$ok" != 1 ]; then
      ...
      stop_pid "$pid"; residual_check "$name/$st-healthfail"; wait_idle_cards "$cards" || true
