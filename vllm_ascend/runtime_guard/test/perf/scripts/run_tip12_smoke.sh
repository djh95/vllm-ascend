...
PRODUCT=/data0/test-mrv2-cann91/rg-tip12-verify
MODEL=/data0/weights/DeepSeek-V2-Lite
ROOT=/data0/test-mrv2-cann91/rg_tip12_smoke
LOG=$ROOT/master.log
mkdir -p "$ROOT"
...
residual_check(){
  # Evidence for orphan processes left after a case stops (zombie-reap attribution).
  local label=$1 out out2
  sleep 5
  out=$(ps -eo pid,ppid,pgid,nlwp,etime,args | grep -E "VLLM::|vllm.entrypoints.openai.api_server|EngineCore" | grep -v grep | head -20)
  if [ -n "$out" ]; then
    log "RESIDUAL_AFTER $label: processes still alive (before force kill):"
    echo "$out" | tee -a "$LOG"
    ...
    pkill -9 -f "vllm.entrypoints.openai.api_serve[r]" 2>/dev/null || true
    sleep 3
    out2=$(ps -eo pid,ppid,pgid,nlwp,etime,args | grep -E "VLLM::|vllm.entrypoints.openai.api_server|EngineCore" | grep -v grep | head -20)
    if [ -n "$out2" ]; then
      log "RESIDUAL_STUCK $label: survived force kill, manual cleanup needed:"
      echo "$out2" | tee -a "$LOG"
    else
      log "RESIDUAL_CLEARED $label: force kill removed all orphans"
    fi
  else
    log "RESIDUAL_AFTER $label: none"
  fi
}
...
run_arm_once(){
  ...
  ( cd "$PRODUCT"
    env PYTHONPATH="$V029:$PRODUCT:${PYTHONPATH:-}" ASCEND_RT_VISIBLE_DEVICES=0,3 VLLM_BATCH_INVARIANT=1 $RP \
    setsid "$PY" -m vllm.entrypoints.openai.api_server \
      ...
      --additional-config "{\"runtime_config\": {\"detector\": {\"logits_finite\": {\"enabled\": true}, \"token_repeat\": {\"enabled\": true, \"window\": 32, \"repeat_sum_threshold\": 64, \"min_tokens\": 32, \"consecutive_hits\": 1}, \"output_substring\": {\"enabled\": true}, \"spec_acceptance\": {\"enabled\": true}}, \"actions\": {\"defaults\": {\"on_trigger\": [\"report\"]}}, \"dump\": {\"dump_dir\": \"$arm/dump\", \"auto_max_times\": 0, \"manual_dump\": 0}}, \"runtime_config_reload_interval\": 2, \"runtime_config_path\": \"$arm/runtime_config.json\", \"runtime_report_dir\": \"$arm/report\"}" \
  ...
  if [ "$ok" != 1 ]; then
    ...
    pkill -9 -f "VLLM::EngineCor[e]" 2>/dev/null || true
    residual_check "$runner-healthfail"
    return 1
  fi
  local out
...
log "=== tip12 smoke start PRODUCT=$PRODUCT vllm029 tip=1992d6b71 cards=0,3 ==="
run_arm_with_retry v2 8143
log "=== DONE ==="
