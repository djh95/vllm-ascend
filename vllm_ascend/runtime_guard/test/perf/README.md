...
|----|---------|----------------|--------|
| **C1** | T0 vs T1 | Guard infra zero-ish overhead when default-off | v2=1.00411 ✅ · v1=0.98466（~1.5%）→ **待重测（等 8 卡空闲排除噪声）** |
| **C2** | T1 vs T2 | Hot-reload poll shell cost | v2=0.99920 ✅ · v1=1.00644 ✅ |
...
| **C6** | Idle leak-back | 压测后 RSS/HBM 回落（见 `perf_lib` leakback） | v2 ✅ 136KB · v1 ⚠️ 受污染 |

> **C3-spec 归因**（`run_c3_spec_tokenrep.sh`）：单独开 `spec_acceptance`+`token_repeat`
> （`output_substring`+`logits_finite` 全程关）→ `T3/T2` **v2=0.99146 ✅** / **v1=0.98798 ❌**（≥0.990）。
> 故 v2 全量失守由 `output_substring` + `logits_finite` 两 detector 的开销导致，
> 非 `spec_acceptance`/`token_repeat`；但 **v1 下 spec_acceptance+token_repeat 也跌破 0.990**，
> 说明 v1 的 detector 开销本身更重，两 detector 并非唯一瓶颈。
