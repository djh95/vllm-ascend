...
def test_resolve_kv_dump_dir_prefers_wave_with_pt_over_empty_arm_wave(tmp_path: Path):
    """BUG-6 residual: arm wave_1 empty, drain lands on wave_2+."""
    dump_root = tmp_path / "dumps"
    req_root = dump_root / "manual_trigger" / "cmpl-ae"
    empty = req_root / "wave_1" / RANK
    empty.mkdir(parents=True)
    hit = req_root / "wave_2" / RANK
    hit.mkdir(parents=True)
    (hit / "L0.pt").write_bytes(b"x")
    report = {
        "incident_type": "manual_trigger",
        "req_id": "__manual_trigger__",
        "dump_dir": str(req_root / "wave_1"),  # legacy arm-wave path
        "dump_arm_wave": 1,
        "rank": RANK,
        "detail": {"requests": [{"req_id": "cmpl-ae"}]},
    }
    assert resolve_kv_dump_dir(report) == hit

    # New product shape: dump_dir is req root (no wave).
    report2 = dict(report)
    report2["dump_dir"] = str(req_root)
    assert resolve_kv_dump_dir(report2) == hit
