...
#    into $GOLDEN_DIR/dump_schema/kv_ref_meta.json by default — that path is
#    INSIDE the git worktree; set RG_META_OUT to stage elsewhere for scratch
#    runs (avoids dirtying the tree with throwaway meta).
...
#   RG_META_OUT=path.json  (default: $GOLDEN_DIR/dump_schema/kv_ref_meta.json)
...
META_OUT="${RG_META_OUT:-$GOLDEN_DIR/dump_schema/kv_ref_meta.json}"
if [[ -z "${RG_META_OUT:-}" ]]; then
  echo "[capture_kv_ref] note: meta lands inside the git worktree ($META_OUT); set RG_META_OUT for scratch runs"
fi

# 1) git-safe meta — kv_ref_meta.json (no tensor data)
python3 - "$WAVE" "$META_OUT" <<'PY'
import json, sys
