...
def test_v18e_file_mode_drain_skips_object_broadcast_when_empty():
    from vllm_ascend.runtime_guard.processor import RuntimeGuardProcessor

    proc = SimpleNamespace(runner=SimpleNamespace(), _kv_dump_jobs=[])
    tp = MagicMock()
    tp.world_size = 2
    tp.rank_in_group = 0
    tp.cpu_group = object()
    ...
    with (
        ...
        RuntimeGuardProcessor._drain_kv_dump(proc)
    tp.broadcast_object.assert_not_called()


def test_v18f_hitchhike_claim_only_tp0_last_pp():
    from vllm_ascend.runtime_guard.processor import RuntimeGuardProcessor

    proc = SimpleNamespace(
        runner=SimpleNamespace(),
        _kv_dump_jobs=[{"req_id": "r1"}],
    )
    with (
        patch(
            "vllm_ascend.runtime_guard.processor.should_dump_kv_on_rank",
        ...
        due, jobs = RuntimeGuardProcessor._claim_kv_dump_jobs(proc)
    assert due is True
    assert jobs == [{"req_id": "r1"}]
...
def test_v19a_manual_dump_forces_all_requests_no_consume_in_prepare(monkeypatch):
    ...
    assert prepared is None  # arm only; D2H is next sync_for_step
...
def test_v19c_empty_block_ids_skips_without_consume(monkeypatch):
    from vllm_ascend.runtime_guard.action.actions import DumpKvAction
    from vllm_ascend.runtime_guard.incident import Incident
    from vllm_ascend.runtime_guard.manual_trigger import MANUAL_TRIGGER_TYPE
...
def test_v19f_multi_job_arm_refunds_once_when_all_fail(tmp_path: Path):
    ...
    try:
        ...
        proc = SimpleNamespace(
            ...
            action_executor=SimpleNamespace(_kv_reader=MagicMock(), _submit_heavy=None),
            quota=quota,
        )
        arm = "arm-shared"
        jobs = [
...
def test_v23b_report_writer_dump_dir_follows_provider(tmp_path: Path):
    ...
    assert record["dump_dir"] == str(root / "token_repeat" / "r-1" / "wave_unknown")
    ...
    assert json.loads(path2.read_text(encoding="utf-8"))["dump_dir"] == str(
        tmp_path / "reports" / "kv_cache" / "token_repeat" / "r-1" / "wave_12"
    )
...
def test_v25_rank_gate_pins_detection_to_tp0(monkeypatch):
    ...
    monkeypatch.setattr(
        rank_gate, "get_tp_group", lambda: SimpleNamespace(rank_in_group=1, world_size=2)
    )
    runner = SimpleNamespace(use_async_scheduling=True, tp_rank=1, tp_size=2)
    assert rank_gate.anomaly_check_rank_skip_reason(runner) == "not TP0"
