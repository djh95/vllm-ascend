...
class MsprobeBridgeMixin:
    ...
    def _reinstall_msprobe_hooks_after_recreate(self) -> None:
        """Re-attach msprobe forward hooks after teardown+init.

        ``_teardown_debugger`` calls ``stop()`` which drops hooks on
        ``runner.model``. ``_init_debugger`` only constructs a new Python
        object; without an immediate ``start(model)`` the lazy path in
        ``start_dump_data`` may never run (dump was disarmed during teardown).
        """
        if self._debugger is None or self._is_aclgraph_dumper():
            return
        ...
        try:
            ...
            logger.warning(
                "[Anomaly msprobe] post-recreate start() failed: %s %s",
                exc,
                self.dump_rank_tag(),
            )
