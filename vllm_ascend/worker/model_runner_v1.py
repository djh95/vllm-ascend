...
class NPUModelRunner(GPUModelRunner):
    ...
    def _record_mtp_acceptance_rate(
        ...
        if draft_len <= 0 or draft_token_ids is None or draft_len != draft_token_ids_len:
            logger.warning(
                "[Garbled-MTP draft len is wrong] req_id=%s draft_len=%d len(draft_token_ids)=%d",
                req_id,
                draft_len,
                draft_token_ids_len,
            )
        ...
        logger.info(
            "[Garbled-MTP short] req_id=%s draft_len=%d effective_draft_len=%d "
    ...
    def _log_mtp_token_details(
        ...
        logger.info(
            "[Garbled-MTP Prompt Tokens] req_id=%s prompt_token_count=%d prompt_token_ids=%s",
            req_id,
            len(prompt_token_ids),
            prompt_token_ids,
        )
        logger.info(
            "[Garbled-MTP Output Tokens] req_id=%s output_token_count=%d output_token_ids=%s",
            req_id,
    ...
    def _disable_msprobe_dump_if_needed(self) -> None:
        ...
        if self._msprobe_dump_disable_delay_rounds > 0:
            ...
            logger.info(
                "[Garbled-MTP msprobe] defer rollback dump_enable for one round. remaining_delay_rounds=%d",
        ...
        dump_cfg = self.ascend_config.dump_config_path
        if not dump_cfg:
            logger.error("[Garbled-MTP msprobe] rollback failed: dump_config_path is empty")
            return
        config_path = Path(dump_cfg)
        if not config_path.exists():
            logger.error(
                "[Garbled-MTP msprobe] rollback failed: config file not found. path=%s",
                str(config_path),
            )
            return
        ...
        try:
            ...
            logger.error(
                "[Garbled-MTP msprobe] rollback failed to read json. path=%s error=%s",
                str(config_path),
                e,
            )
            return
        ...
        if not isinstance(config_obj, dict):
            logger.error(
                "[Garbled-MTP msprobe] rollback failed: json root is not object. path=%s type=%s",
                str(config_path),
        ...
        try:
            if self.debugger is None:
                logger.info(
                    "[Garbled-MTP msprobe] rollback dump_enable to %s after "
            ...
            logger.info(
                "[Garbled-MTP msprobe] rollback dump_enable from %s to restore_value=%s after debugger.step. path=%s",
                curr_value,
                self._msprobe_dump_restore_value,
                str(config_path),
            )
            ...
            logger.error(
                "[Garbled-MTP msprobe] rollback failed to write json. path=%s error=%s",
                str(config_path),
                e,
            )
    ...
    def _enable_msprobe_dump_if_needed(self, req_id: str) -> bool:
        ...
        dump_cfg = self.ascend_config.dump_config_path
        if not dump_cfg:
            logger.error("[Garbled-MTP msprobe] req_id=%s skip dump: dump_config_path is empty", req_id)
            return False
        if req_id in self._msprobe_dumped_req_ids:
            logger.info_once("[Garbled-MTP msprobe] req_id=%s skip dump: request already dumped once", req_id)
            return False
        if self._msprobe_dump_total_count >= self._msprobe_dump_max_times:
            logger.info_once("[Garbled-MTP msprobe] req_id=%s skip dump: reached max dump times=%d",
        ...
        if elapsed is not None and elapsed < self._msprobe_dump_cooldown_seconds:
            logger.info_once(
                "[Garbled-MTP msprobe] req_id=%s skip dump: cooldown active elapsed=%.2fs remaining=%.2fs",
                req_id,
                elapsed,
        ...
        config_path = Path(dump_cfg)
        if not config_path.exists():
            logger.error("[Garbled-MTP msprobe] req_id=%s config file not found. searched_path=%s", req_id,
                         str(config_path))
            return False
        try:
            with config_path.open("r", encoding="utf-8") as f:
        ...
        if old_value is True:
            logger.info_once(
                "[Garbled-MTP msprobe] req_id=%s skip dump: dump_enable already true. path=%s",
                req_id,
                str(config_path),
            )
            return True
        ...
        try:
            ...
            self.debugger._maybe_reload_config(force=True)
            logger.info(
                "[Garbled-MTP msprobe] req_id=%s set dump_enable to %s. changed_by_runner=%s dump_count=%d/%d path=%s",
                req_id,
