...
class NPUModelRunner(GPUModelRunner):
    ...
    def _compute_logits_with_guard(self, sample_hidden_states, scheduler_output, positions):
        ...
        Stashes ``scheduler_output`` for the wrap (post ngram_gpu replace) and
        wraps compute_logits only when a pre-sample detector is active.
        ``positions`` is unused (kept for call-site compatibility).
        """
        del positions
    ...
    def sample_tokens(
        ...
        # Post-pre-sample hooks (ensure_logprobs → … → check_after_sample)
