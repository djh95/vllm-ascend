...
class EplbWorker:
    ...
    def do_update(self):
        ...
        self.latest_map_consistency = {
            ...
            "num_valid_experts": int(placement_stats.get("num_valid_experts", 0)),
        }
        self.latest_rebalance_result = {
    ...
    def check_expert_placement(self, old_placement, new_placement):
        ...
        for layer_id in range(num_layers):
            layer_valid = int(torch.unique(new_placement[layer_id][new_placement[layer_id] >= 0]).numel())
            num_valid_experts += layer_valid
        ...
        return {
            ...
            "num_valid_experts": num_valid_experts,
        }

    @staticmethod
    def _compute_load_balance(load_info: Any) -> dict[str, float]:
        """Aggregate per-rank token load into avg/max/balancedness gauges."""
        if load_info is None or not torch.is_tensor(load_info) or load_info.numel() == 0:
            return {"avg_tokens": 0.0, "max_tokens": 0.0, "balancedness": 0.0}
