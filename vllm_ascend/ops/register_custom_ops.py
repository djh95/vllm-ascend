...
def _maybe_all_gather_and_maybe_unpad_impl(x: torch.Tensor, label: bool, is_ep_comm: bool = False) -> torch.Tensor:
    ...
    if flash_comm_v1_enabled and label:
        from vllm_ascend.observability.flashcomm_stats import OP_ALL_GATHER, CollectiveTimer

        dp_metadata = forward_context.dp_metadata
        with CollectiveTimer(OP_ALL_GATHER, x):
...
def _maybe_pad_and_reduce_impl(x: torch.Tensor, is_ep_comm: bool = False) -> torch.Tensor:
    ...
    from vllm_ascend.observability.flashcomm_stats import OP_REDUCE_SCATTER, CollectiveTimer

    dp_metadata = forward_context.dp_metadata
    with CollectiveTimer(OP_REDUCE_SCATTER, x):
        if dp_metadata is None or not is_ep_comm:
            pad_size = _EXTRA_CTX.pad_size
            if pad_size > 0:
