...
2. **Sync scope never spans idle-asymmetric DPs**
   ...
   Dump jobs hitchhike on the broadcast-group ``all_reduce`` when that group
   exists (PP=1). File / PP>1 dump uses a separate last-PP TP ``all_reduce``.
...
class RuntimeConfig:
    ...
    def sync_runtime_config(self) -> bool:
        ...
        changed, _extra = self.sync_with_hitchhike()
        return changed

    def sync_with_hitchhike(
        self,
        extra_due: bool = False,
        extra: Any = None,
    ) -> tuple[bool, list[Any]]:
        """Config sync; dump jobs may ride the same all_reduce + broadcast_object.

        Broadcast: every rank in the **per-DP** sync group must call this each
        step and always ``all_reduce([config_due, extra_due], MAX)`` — never
        wall-clock-skip the collective. File I/O + object broadcast run only
        when either bit is due. Dump-only due does not advance the config
        interval. File mode (no group / PP>1) returns ``(local_reload, [])``;
        the caller must sync dump on the last-PP TP group itself.
        """
        if not self.hot_reload_enabled:
            return False, []
        if self.sync_mode == SYNC_BROADCAST:
            ...
            if group is not None and group.world_size > 1:
                return self._maybe_reload_broadcast(
                    group, extra_due=extra_due, extra=extra
                )
    ...
    def _maybe_reload_broadcast(
        ...
        *,
        ...
        Every call ``all_reduce([config_due, extra_due], MAX)`` so ranks cannot
        diverge on whether to enter the object broadcast. File I/O stays on
        ``sync_group`` first rank and only runs when **config** is due.
        ``extra`` (dump jobs) hitchhikes on the same ``broadcast_object`` when
        the extra bit is set; dump-only must not advance the config interval.
        """
        import torch
        ...
        config_due_local = (
            1.0
            if ((not self._initial_broadcast_done) or (now - self._last_reload_ts >= interval))
            else 0.0
        )
        extra_due_local = 1.0 if extra_due else 0.0
        role = _process_role_tag()
        logger.debug(
            "[runtime_config sync] enter stage=config_all_reduce "
            "config_due_local=%.0f extra_due_local=%.0f initial_done=%s group_size=%s %s",
            config_due_local,
            extra_due_local,
        ...
        due_t = torch.tensor([config_due_local, extra_due_local], dtype=torch.float32)
        torch.distributed.all_reduce(
            due_t,
            op=torch.distributed.ReduceOp.MAX,
            group=sync_group.cpu_group,
        )
        config_due = float(due_t[0].item())
        dump_due = float(due_t[1].item())
        logger.debug(
            "[runtime_config sync] leave stage=config_all_reduce config_due=%.0f extra_due=%.0f %s",
            config_due,
            dump_due,
            role,
        )
        if config_due < 0.5 and dump_due < 0.5:
            return False, []

        changed = False
        payload: dict[str, Any] | None = None
        if sync_group.is_first_rank:
            dump_jobs = list(extra or []) if dump_due >= 0.5 else []
            if config_due >= 0.5:
        ...
        dump_jobs = list((payload or {}).get("dump_jobs") or []) if isinstance(payload, dict) else []
        if payload is None or not isinstance(payload, dict):
            return False, dump_jobs
        if payload.get("skip_config") or config_due < 0.5:
            return False, dump_jobs
        first_sync = not self._initial_broadcast_done
        ...
        if data is None:
            ...
            config_changed = False if not sync_group.is_first_rank else (changed or first_sync)
            return config_changed, dump_jobs
        ...
        if not sync_group.is_first_rank:
            if version != self._version:
                ...
                try:
                    return self._apply_loaded(data, version=version), dump_jobs
                except Exception as exc:
                    logger.error(
        ...
        return (changed or first_sync), dump_jobs
    ...
    def _apply_loaded(
        ...
        *,
        ...
        if self._sync_mode_frozen is not None:
            ...
            self._sync_mode_frozen = mode if mode in (SYNC_BROADCAST, SYNC_FILE) else SYNC_BROADCAST
        validate_runtime_config(merged)
