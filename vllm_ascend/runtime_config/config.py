...
class RuntimeConfig:
    ...
    def detector_placement_mode(self) -> str:
        sec = self._data.get("detector_placement") or {}
        mode = str(sec.get("mode", "auto") or "auto")
        return mode if mode in ("auto", "manual") else "auto"

    def detector_placement_manual(self) -> dict[str, int]:
        sec = self._data.get("detector_placement") or {}
        raw = sec.get("manual") or {}
        out: dict[str, int] = {}
    ...
    def detector_placement_pin(self) -> bool:
        sec = self._data.get("detector_placement") or {}
        return bool(sec.get("pin", False))
    ...
    def _maybe_reload_broadcast(self, sync_group) -> bool:
        ...
        When clearly before the reload interval (skew-budgeted), skip the
        collective entirely — all ranks take this path together after a shared
        prior sync. Near the boundary, ``all_reduce(due)`` keeps followers in
        ...
        now = time.time()
        interval = self.reload_interval_seconds
        if self._initial_broadcast_done and interval > 0:
            # Far from due → no collective. Near the edge (within skew budget)
            # fall through so clock skew cannot split the group.
            early_by = interval - _BROADCAST_DUE_SKEW_SEC
            if early_by > 0 and (now - self._last_reload_ts) < early_by:
                return False
