...
def random_sample(
    ...
    with npu_stream_switch(global_stream()):
        ...
        if len(generators) == total_seq:
            ...
            # 全局批量采样打底
            global_batch_seed = random.randint(1, 2**31 - 1)
...
class AscendTopKTopPSampler(TopKTopPSampler):
    ...
    def forward_native(self, logits, generators, k, p):
        ...
        if get_ascend_config().enable_reduce_sample:
            ...
            if self.top_k and k and self.top_k < k:
                logger.warning_once(
                    "[sample/sampler] local top_k=%d < k=%d, potential Top-K error may occur.", self.top_k, k
                )
