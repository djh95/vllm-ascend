...
def test_v23b_report_writer_dump_dir_follows_provider(tmp_path: Path):
    ...
    path2 = w2.write(incident_type="token_repeat", req_id="r-1", detail={"x": 1})
    assert json.loads(path2.read_text(encoding="utf-8"))["dump_dir"] == str(
        tmp_path / "reports" / "kv_cache" / "token_repeat" / "r-1"
    )
...
def test_v25_placement_lpt_balance_deterministic_all_scope():
    ...
    specs = [
        ...
        _spec("output_substring", cost=1.0),
    ...
    # gives rank0={logits_finite(2)+token_repeat(0.5)} rank1={token_logprob(1.5)+output_substring(1)}.
    ...
    p3 = plan_placement([_spec("rank_local_det", rank_local=True)], tp_size=2, rank_local_world=True)
    assert p3.runs_here("rank_local_det", 0) and p3.runs_here("rank_local_det", 1)
    p4 = plan_placement([_spec("rank_local_det", rank_local=True)], tp_size=2, rank_local_world=False)
...
def test_v25b_manager_gates_stages_by_placement(tmp_path, monkeypatch):
    ...
    out0 = rank0.check_before_sample(logits=None, input_batch=None)
    out1 = rank1.check_before_sample(logits=None, input_batch=None)
