...
def instrumented_dfx():
    ...
    proc.run_sample_phase = DfxProcessor.run_sample_phase.__get__(proc)
    return proc
...
def _import_npu_model_runner():
    """Import NPUModelRunner with release/vLLM-compat stubs.

    ``releases/v0.23.0`` runners pull in NPU/vLLM symbols that are absent from
    lightweight UT envs (no ``torch_npu``, older ``vllm.v1.worker.cp_utils``).
    Stub the minimum surface so this contract test can bind the real
    ``sample_tokens`` method without a full NPU stack. If import still fails,
    callers should xfail rather than rewrite the runner.
    """
    import sys
    import types
    ...
    if "torch_npu" not in sys.modules:
        ...
        torch_npu.npu.current_device = MagicMock(return_value=0)
        torch_npu._npu_flash_attention = MagicMock()
        sys.modules["torch_npu"] = torch_npu
        sys.modules["torch_npu._inductor"] = MagicMock()

    try:
        from vllm.v1.worker import cp_utils as _cp_utils
        if not hasattr(_cp_utils, "get_total_cp_world_size"):
            _cp_utils.get_total_cp_world_size = lambda: 1  # type: ignore[attr-defined]
    except Exception:
        mod = types.ModuleType("vllm.v1.worker.cp_utils")
        mod.get_total_cp_world_size = lambda: 1  # type: ignore[attr-defined]
        sys.modules.setdefault("vllm.v1.worker.cp_utils", mod)

    from vllm_ascend.worker.model_runner_v1 import NPUModelRunner
    return NPUModelRunner


def _require_npu_model_runner():
    """Return NPUModelRunner or xfail when the release runner cannot import."""
    try:
        return _import_npu_model_runner()
    except Exception as exc:  # pragma: no cover - env-dependent
        pytest.xfail(
            f"NPUModelRunner unavailable in this UT env (release/vLLM skew): {exc}"
        )
...
def _build_minimal_runner_mock(instrumented_dfx, *, use_async=False, spec=False):
    ...
    """
    _RunnerCls = _require_npu_model_runner()
    ...
    runner.ascend_config.scheduler_config.profiling_chunk_config = MagicMock(enabled=False, need_timing=False)
